...
sub split_reservations {
  my $tokens_ref = fetch_tokens ();
  my %seen_variables = ();
  my @tokens = @{$tokens_ref};
  my @reservations = ();
  my $reservation = '';
  my ($scanning_identifiers, $scanning_reservation_block, $scanning_type) 
    = (0,0,0);
  my $ignore_initial_comma = 0;
  my $token;
  my $num_tokens = $#tokens;
  for ($i = 0; $i < $num_tokens; $i++) {
    $token = $tokens[$i];
    warn ("looking at token $token\n");

    if ($token eq 'reserve') {
      warn ("starting to look at reservations");
	...
	warn ("scanning for a type; current token is $token and the next token is $next and the one after that is $after_next\n");
	
	if ($next eq ';') { # done with the entire reservation block
	  warn ("Wrapping up entire reservation block: $reservation\n");
	  push (@reservations, $reservation);
	  $reservation = '';
	  $scanning_reservation_block = 0;
	  $scanning_identifiers = 0;
	  $scanning_type = 0;
	  warn ("done with a reservation\n");
	} elsif ($next eq 'of' or $next eq 'over') { # dependent type
	  warn ("we're looking at a dependent type\n");
	  ...
	  $token = $tokens[$i];
	  warn ("scanning for a term; looking now at $token\n");
	  ...
	  if ($after_next eq ';') {
	    $scanning_reservation_block = 0;
	    warn ("done looking at reservations\n");
	  }

	} else { # non-dependent type
	  $scanning_type = 0;
	  $scanning_identifiers = 1;
	  $ignore_initial_comma = 1;
	  push (@reservations, $reservation);
	  warn ("Wrapping up a non-dependent reservation: $reservation\n");
	  $reservation = '';
	}
      }	elsif ($scanning_identifiers) {
	warn ("scanning identifiers; current token is $token\n");
	if ($token eq 'for') {
	  warn ("done scanning identifiers\n");
	  $scanning_identifiers = 0;
	  $scanning_type = 1;
	  $reservation .= ' ' . $token . ' ';
	} elsif ($token eq ',') {
	  if ($ignore_initial_comma) {
	    $ignore_initial_comma = 0;
	  } else {
	    $reservation .= ',';
	  }
	} else {
	  $seen_variables{$token} = 0;
	  $reservation .= ' ' . $token;
	}
      }
    }
